...
export async function reviewPendingRegionalEpsu(
  epsuId,
  status,
  logoPath = null,
  hostEmail = null,
  ...
  if (
    ...
    if (
      ...
      normalizeEmail(options.existingHostEmail) &&
      normalizeEmail(hostEmail) !== normalizeEmail(options.existingHostEmail)
    ) {
      throw new Error('This backend still uses the older approval flow and cannot assign a different future host yet');
    }

    console.log('[school-api] falling back to legacy regional approval RPC', {
      epsuId,
      title: options.title,
      countryCode: options.countryCode,
    });
    const legacyResult = await reviewRegionalEpsuSuggestion(
      options.title,
      options.countryCode,
      status,
      logoPath
...
export async function reviewPendingSchoolEpsu(
  epsuId,
  status,
  logoPath = null,
  hostEmail = null,
  ...
  if (isMissingRpcSignature(error)) {
    ...
    console.log('[school-api] falling back to legacy school approval RPC', {
      epsuId,
      status,
    });
    const legacyCall = await supabase.rpc('review_pending_school_epsu', {
      p_epsu_id: epsuId,
      p_status: status,
      p_logo_path: logoPath,
    });
    ...
    if (legacyCall.error) {
      throw legacyCall.error;
    }

    return legacyCall.data;
  }
